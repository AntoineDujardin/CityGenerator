...
if "ResidentialHouseBlock" in locals():
    import imp
    imp.reload(block)
    imp.reload(const)
    imp.reload(parcel)
...
class ResidentialHouseBlock(block.Block):
    ...
    def parcel(self):
        ...
        max_corner_size = min(
            (self.parcels_x_size - const.min_residential_houses_size)/2,
            (self.parcels_y_size - const.min_residential_houses_size)/2,
